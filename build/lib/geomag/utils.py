...
def double_factorial(n):
    ...
    """
    assert n > 0 and int(n) == n, "Invalid input to double factorial"
    start_number = 2 - (n % 2)
...
def legendre_constants(size):
    ...
    return tuple(tuple(lst) for lst in k)


def calculate_decimal_year(date_of_year):
...
def schmidt_quasi_normalisation(max_n):
    ...
    for n in range(max_n):
        for m in range(n + 1):
            ...
            double_fact = 1.0 if n == 0 else double_factorial(2 * n - 1)
            k_delta = kronecker_delta(0, m)
            n_minus_m_fact = math.factorial(n - m)
            n_plus_m_fact = math.factorial(n + m)
            schmidt[m][n] = (
                math.sqrt(((2 - k_delta) * n_minus_m_fact) / n_plus_m_fact)
                * double_fact
                / n_minus_m_fact
            )
    return schmidt
    ...
    for n in range(1, max_n):
        for m in range(n + 1):
            if m == n:
                ...
                derivative_associated_poly[m][n] = (
                    cos_theta * derivative_associated_poly[m - 1][n - 1]
                ...
                associated_poly[m][n] = (
                    sin_theta * previous_ass_poly
                    - current_k * associated_poly[m][n - 2]
                )
                derivative_associated_poly[m][n] = (
                    sin_theta * derivative_associated_poly[m][n - 1]
...
def scalar_potential(
    coeff, phi, theta, max_n, max_m, radial_alt, ref_radius=6371200, k=None
):
    """
    ...
    legendre_poly, legendre_poly_derivative = associated_polynomials(
        cos_theta, sin_theta, max_n, max_m, k
    )
...
def _calc_scalar_potential(
    ...
    for n in range(1, max_n):
        ...
        for m in range(n + 1):
            ...
            B_r += (
                current_a_over_r_power * (n + 1) * (g_cos + h_sin) * legendre_poly[m][n]
            )
            B_theta -= (
                current_a_over_r_power * (g_cos + h_sin) * legendre_poly_der[m][n]
            )
...
def contour_to_geojson(
    ...
    from geojsoncontour.utilities.multipoly import keep_high_angle
    ...
    for collection in collections:
        for path in collection.get_paths():
            ...
            coordinates = (
                np.around(coordinates, ndigits) if ndigits is not None else coordinates
            )
            ...
            properties = {
                ...
                "level-display": "{}{}".format(
                    round(abs(val)), "°E" if val > 0 else "°W"
                ),
            }
    ...
    return _render_feature_collection(
        feature_collection, geojson_filepath, strdump, serialize
    )


def _render_feature_collection(
    feature_collection, geojson_filepath, strdump, serialize
):
    if not serialize:
